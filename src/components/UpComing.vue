      ...
      <div class="card" v-for="dt in data" :key="dt.id">
        ...
        <div class="wrapper-detail">
          <p class="info">
            ...
            <span class="release-date"> {{ dt.release_date.slice(0, 4) }}</span>
          </p>
          <p class="title">{{ dt.original_title }}</p>
        </div>
      </div>
      <div v-if="loading">
        <HollowDotsSpinner
...
export default {
  ...
  methods: {
    async getDataUpComingMovies() {
      await axios.get(upComing + "&language=en-US&page=1").then((res) => {
